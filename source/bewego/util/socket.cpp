...
int Socket::bind(string ip, string port) {
  ...
  if ((status = getaddrinfo(ip.c_str(), port.c_str(), &address_info, &res)) !=
      0) {
...
Socket *Socket::accept() {
  ...
  Socket *newSocket =
      new Socket(address_info.ai_family, address_info.ai_socktype,
                 address_info.ai_protocol);
  newSocket->sock = newsock;
  newSocket->port = port;

  char host[NI_MAXHOST];
  int status = getnameinfo((struct sockaddr *)&their_addr, sizeof(their_addr),
...
int Socket::socket_writeTo(string msg, string ip, string port) {
  ...
  address_info.ai_addr = res->ai_addr;
  freeaddrinfo(res);
  status = (int)sendto(sock, buf, len, 0, address_info.ai_addr,
...
int Socket::socket_readFrom(string &buf, int len, string ip, string port) {
  ...
  address_info.ai_addr = res->ai_addr;
  freeaddrinfo(res);
  status = (int)recvfrom(sock, buffer, len - 1, 0, address_info.ai_addr,
                         &address_info.ai_addrlen);
  if (status < 0) {
    // exit(1);
...
int Socket::select(vector<Socket> *reads, vector<Socket> *writes,
                   vector<Socket> *exceptions, int seconds) {
