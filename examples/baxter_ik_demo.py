...
def setUpWorld(initialSimSteps=100):
    ...
    # Load plane
    p.loadURDF(DATADIR + "plane.urdf", [0, 0, -1], useFixedBase=True)
    ...
    # p.resetBasePositionAndOrientation(baxterId,
    # [0.5, -0.8, 0.0],[0,0,0,1])
    # p.resetBasePositionAndOrientation(baxterId,
    # [0, 0, 0], )
...
def accurateIK(bodyId, endEffectorId, targetPosition,
               lowerLimits, upperLimits, jointRanges, restPoses,
    ...
    while (not closeEnough and iter < maxIter):
        ...
        newPos = ls[4]
        diff = [targetPosition[0] - newPos[0],
...
if __name__ == "__main__":
    ...
    lowerLimits, upperLimits, jointRanges, restPoses = getJointRanges(
        baxterId, includeFixed=False)

    # targetPosition = [0.2, 0.8, -0.1]
    # targetPosition = [0.8, 0.2, -0.1]
    targetPosition = [0.2, 0.0, -0.1]

    p.addUserDebugText("TARGET", targetPosition,
