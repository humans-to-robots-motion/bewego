...
class IpoptProblemWrapper(object):
    ...
    def __init__(
            self, fun, args=(), kwargs=None, jac=None, hess=None, hessp=None,
            hess_structure=None,
        ...
        self.last_x = None
        self.hess_structure = hess_structure
        ...
        for con in constraints:
            ...
            if con_jac is None:
                con_fun = FunctionWithApproxJacobian(
                    con_fun, epsilon=eps, verbose=False)
                con_jac = con_fun.jac
    ...
    def hessianstructure(self):
        if self.hess_structure is not None:
            return self.hess_structure()
        return [], []

    def hessian(self, x, lagrange, obj_factor):
        hess_l = self.hess(x)
        hess = obj_factor * hess_l[0]
        for i in range(len(lagrange)):
            hess += lagrange[i] * hess_l[i + 1]
        rol, col = self.hessianstructure()
        return hess[rol, col]
...
def minimize_ipopt(fun, x0, args=(), kwargs=None, method=None,
                   jac=None, hess=None, hessp=None, hess_structure=None,
    ...
    problem = IpoptProblemWrapper(
        fun, args=args, kwargs=kwargs,
        jac=jac, hess=hess,
        hessp=hessp, hess_structure=hess_structure,
        constraints=constraints)
    ...
    for option, value in options.items():
        try:
            ...
            raise TypeError(
                '''Invalid option for IPOPT: {0}: {1} 
                   (Original message: "{2}")'''.format(option, value, e))
